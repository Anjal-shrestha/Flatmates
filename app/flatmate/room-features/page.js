...
export default function Features() {
  ...
  const { selectedBedSize, selectedItems } = flatDetails;
  console.log(flatDetails);

  const handleBedSizeClick = (bedSize) => {
    updateFlatDetails({ selectedBedSize: bedSize });
  };
  ...
  const handleButtonClick = (item) => {
    const updatedItems = selectedItems.includes(item)
      ...
      : [...selectedItems, item];
    updateFlatDetails({ selectedItems: updatedItems });
  };
  ...
  return (
    <main className="py-7">
      <div className="ml-64 mx-10">
        ...
        <h3 className="text-lg leading-6 font-medium text-blue-900">Room Features</h3>
      </div>
      <div className="flex items-center justify-center mt-5">
        <div className="bg-gray-000 shadow-md rounded-md p-4 max-w-lg w-full">
          <div className="px-4 sm:px-4 mt-2">
            <label className="block text-xs font-medium text-gray-700">Bed Size</label>
            <div className="flex mt-1 space-x-2">
              {["Single", "Double", "Queen", "King", "None"].map((item) => (
                <button
                  key={item}
                  className={`px-5 py-1 text-xs border ${
                    selectedBedSize === item ? "bg-gray-200 border-blue-400" : "border-gray-400"
                  }`}
                  onClick={() => handleBedSizeClick(item)}
                >
                  {item}
                </button>
              ))}
